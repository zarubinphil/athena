---
name: self-learning
description: Самообучение Athena OS на чекпойнте/в конце сессии — разбор ошибок (причина→фикс→системное правило), token-аудит (что съело бюджет, как сэкономить дальше), фиксация данных о владельце и повторяющихся паттернов, обновление routing-лога по исходу. Триггеры: "конец сессии", "ретро", "разбор сессии", "session review", "чему научились", закрытие фазы, перед хэндоффом.
metadata:
  model: Sonnet
---

# Самообучение (self-learning) — Athena OS

Превращает каждую сессию в компаундящееся знание. Метод Карпаты: **синтез на записи**
(чистим и обобщаем урок в момент записи, не храним сырьё), **одна метрика делает петлю умной**,
...
Дёшево по токенам: это короткий структурированный проход, не отдельное исследование.
...
- `token-audit-log.md` — куда ушёл бюджет и как сэкономить в следующий раз.
...
- Конкретная оптимизация на следующий раз (разведка-first, субагент на тяжёлое чтение,
