---
name: athena-update
description: Безопасное обновление уже-установленной Athena OS, когда вышла новая версия каркаса. Запускать когда у человека система УЖЕ стоит (прошёл setup-os), а репо athena обновился — новые слои/шаблоны/манифесты/фиксы. Тянет апдейт в живую систему без потери личных правок: анализ структуры → план изменений → согласование попапами → действие с бэкапом и откатом. Триггеры: "обнови athena", "athena update", "вышла новая версия системы", "подтяни обновления каркаса", "update os".
---

# athena-update — обновить живую систему безопасно

> Антипод `setup-os` (ставит с нуля). Здесь система УЖЕ есть — задача подтянуть новую версию каркаса, **не затерев личное**. Дирижируешь ТЫ; `bootstrap.sh` остаётся тупым идемпотентным движком.
...
|---|---|---|---|
| **Движок/слои** | `bootstrap.sh`, `Brewfile`, `*.manifest` | пере-прогон затронутых слоёв (идемпотент) | новые `brew install`, новые клоны |
...
- **Локальные правки репо** (`git status` непусто / ветка разошлась) → `--ff-only` упадёт. НЕ форсить — вынести в План как блокер, дать выбор (stash/ветка/отмена).
- **Дрейф живого Сознания** (`chezmoi diff` непусто) → у юзера есть ручные правки `~/.claude`, которые apply сотрёт. Показать их в Плане пофайлово.
...
Если позади 0 коммитов И `chezmoi diff` пуст → «всё актуально», стоп.
...
4. Если репо разошёлся — попап: [Stash мои правки → обновить → вернуть] [Отдельная ветка] [Отмена].
...
# 3. Прогон затронутых слоёв: сперва dry-run (покажи), потом реально
