...
**ОПУБЛИКОВАН:** `github.com/zarubinphil/athena` (public, MIT). Автор истории «Зарубин Филипп».
- [x] **P0.1** gitignore (`athena.config.sh`/`chezmoidata.yaml`).
- [x] **P0.2** sanitize tracked + smoke grep-гейт `[личное]` (доказан RED-инъекцией).
- [x] **P0.3** git identity = канон zarubinphil; история переподписана filter-branch.
