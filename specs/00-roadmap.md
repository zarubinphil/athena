...
## Фаза 8 — Dashboard (thin UI over verified local workflows)
Тонкий UI-слой поверх Ф7 local-workflows. Без собственной логики — только отображение состояния job-ledger + маршрутизации.
- [ ] Spec + грилл (входные данные: job-lifecycle.yaml + routing-evals.jsonl).
- [ ] Реализация: web/TUI — после live-bootstrap (Ф5 P1) или параллельно.
