...
`vault-znaniya` + `athena-private` (overlay) приватные/pushed (zarubinphil). Merged-source,
...
- [x] **Манифест проектов → athena-private:** `projects.manifest` (zarubinphil/Mnemazine→mnemazine, zarubinphil/themis→themis). Живые разбросанные копии не трогаются (FS-реорг отдельно).
...
- [x] **dry-validate (эмуляция, без chezmoi):** `smoke/dry-validate.sh` — merge→temp, рендер известных vars, plutil/json/bash-n + лов неизвестных `{{ }}`. Зелёный.

**Остаток до полного DoD:** commit+push обоих репо (приват small → без chunking) · live Keychain-миграция (владелец, после ротации tg) · клон telegram-бота в `~/tools` (URL/добро отдельно). Истинный `chezmoi execute-template` на dest → Ф5.
