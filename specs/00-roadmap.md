...
- [x] Шаг 1 — `chezmoi/dot_agents/role-passports/` (7 паспортов English + `_index.md`, канон-формат).
- [x] Шаг 2 — `chezmoi/dot_agents/handoff-graph.yaml` (машиночитаемый: agents/handoffs/forbidden/gates + learning-tail).
- [x] Шаг 3 — `smoke/agent-contract.sh` (passports + handoff integrity; негатив-тесты ловят when/why/tail/passport; врезан в smoke.sh).
