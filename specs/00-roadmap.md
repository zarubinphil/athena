...
- [x] **commit+push:** athena-os `b22056f` (0 remote) · athena-private `eef1d60` (pushed zarubinphil).
- [x] **Слой 0b (tools):** `tools.manifest` (+`.example`) → клон `~/tools/*` ДО Мозга (run_once_ найдёт бота). tg-бот автоматизирован (public `RichardAtCT/claude-code-telegram` + `poetry install`); `poetry` в Brewfile.
